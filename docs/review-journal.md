# Review Journal

This journal records the domain cases that matter before widening the public API.

The local checks classify each case as `ship`, `watch`, or `hold`. That gives the project a small review vocabulary that matches its databases focus without claiming live deployment or external usage.

## Cases

- `baseline`: `index fit`, score 176, lane `ship`
- `stress`: `join width`, score 134, lane `watch`
- `edge`: `constraint risk`, score 207, lane `ship`
- `recovery`: `plan drift`, score 164, lane `ship`
- `stale`: `index fit`, score 163, lane `ship`

## Note

This file is intentionally plain so the fixture remains the source of truth.
